...
BOWTIE_l = 20   # First `L` bases (L > 4) on the high-quality (left) end of the read.
BOWTIE_e = 300  # Sum of PHRED qualities at all mismatched positions
...
class EM(object):
    ...
    def do_mapping_bowtie(self, full_fasta_path, nice = None):
        ...
        if self.reads2_filepath is not None:
            bowtie_command = "%s %s | %s bowtie %s --minins %d --maxins %d %s -1 - -2 %s | samtools view -b -S -F 0x0004 - | samtools sort - > %s.PE.bam 2> %s "%(\
                cat_cmd,
            ...
            bowtie_command = "%s %s | %s bowtie %s %s - | samtools view -b -S -F 0x0004 - | samtools sort - > %s.PE.bam 2> %s "%(\
                cat_cmd,
                self.reads1_filepath,
                nicestring,
                shared_bowtie_params,
...
def do_initial_mapping(working_dir, options):
    ...
    if options.fastq_reads_2 is not None:
        ...
        cmd = "%s %s | %s bowtie --phred%d-quals -t -p %s -n 3 -l %s -e %s --best --sam --chunkmbs 128 --minins %s --maxins %s %s -1 - -2 %s | samtools view -b -S -u -F 0x0004 - | samtools sort - > %s.bam "%tuple(option_strings)
    # SINGLE END MAPPING
    else:
        option_strings.extend([options.bowtie_db, bampath_prefix])
        cmd = "%s %s | %s bowtie --phred%d-quals -t -p %s -n 3 -l %s -e %s --best --sam --chunkmbs 128  %s - | samtools view -b -S -u -F 0x0004 - | samtools sort - > %s.bam "%tuple(option_strings)
